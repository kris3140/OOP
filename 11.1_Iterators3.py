...
class Klanten(Klant):
    ...
    def __next__(self):
        ...
        gegevens = f"{key} : saldo: {self.dict[key][0]} / verzekering: {self.dict[key][1]} "
        return gegevens

    def add_klant(self, naam, saldo=0, verzekering='ja'):
        self.keys_list.append(naam)
        self.dict.update( {naam : [saldo, verzekering] } )
        return self.dict


mijnklanten = Klanten()
mijnklanten.add_klant("Peeters", 100, "nee")
